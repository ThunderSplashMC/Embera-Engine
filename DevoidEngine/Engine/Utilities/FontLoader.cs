...
namespace DevoidEngine.Engine.Utilities
{
    static class FontLoader
    {
        ...
        public static Texture LoadFace(string path)
        {
			Library library = new Library();
			FontService FontService = new FontService();
			FontService.SetFont(new Face(library, path));
			FTBitmap bmp = FontService.RenderString("Hello World");

			return new Texture(bmp.BufferData);
        }
    }
	...
	internal class FontService : IDisposable
	{
		...
		internal virtual FTBitmap RenderString(string text)
		{
			try
			{
		...
		/// <returns></returns>
		internal virtual FTBitmap RenderString(string text, Color foreColor)
		{
		...
		/// <returns></returns>
		internal virtual FTBitmap RenderString(string text, Color foreColor, Color backColor)
		{
			return RenderString(this.lib, this.FontFace, text, foreColor, backColor);
		}

		internal static FTBitmap RenderString(Library library, Face face, string text, Color foreColor, Color backColor)
		{
			...
			FTBitmap ftbmp = new FTBitmap(library);
			...
			for (int i = 0; i < text.Length; i++)
			{
				...
				rc.RightEdge = penX;
				spacingError = ftbmp.Width - (int)Math.Round(rc.RightEdge);
