...
void main()
{
    ...
    worldRay.Direction = GetWorldSpaceDirection(inverse(basicDataUBO.Projection), inverse(basicDataUBO.View), ndc);

    float t1, t2;
    if (!(RayCuboidIntersect(worldRay, vec3(-64, -64, -64), vec3(64, 64, 64), t1, t2) && t2 > 0.0))
    {
...
vec4 TraceCone(vec3 start, vec3 direction, float coneAngle, float stepMultiplier)
{
    vec3 voxelGridWorlSpaceSize = vec3(64, 64, 64) - voxelizerDataUBO.GridMin;
